...
# Reverse mapping from pattern to pattern type
pattern_to_type = {
    pattern: ptype
...
def render_top_controls():
    ...
    # Top row with filter icon (optional future use)
    top_row = st.columns([10, 1])
    with top_row[1]:
        st.button("🧰", help="Filter (Coming soon)", disabled=True)
    ...
    return duration, st.session_state.pattern_type, st.session_state.pattern_selected, scan_clicked
